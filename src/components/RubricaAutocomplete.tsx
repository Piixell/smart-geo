  ...
  return (
    <div ref={wrapperRef} className="relative">
      <div className="relative">
        ...
        {loading && (
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-signal-500"></div>
          </div>
        )}
      </div>
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-ink-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          {suggestions.map((contatto, index) => (
            <div
              ...
              className={`px-4 py-3 cursor-pointer border-b border-ink-100 last:border-b-0 transition-colors ${
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex-shrink-0">
                  <div className="w-8 h-8 rounded-full bg-signal-100 flex items-center justify-center">
                    <User className="w-4 h-4 text-signal-600" />
                  </div>
                </div>
      ...
      {showDropdown && value.length >= 2 && suggestions.length === 0 && !loading && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-ink-200 rounded-lg shadow-lg">
